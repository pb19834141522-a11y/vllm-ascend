...
class AscendConfig:
    ...
    def __init__(self, vllm_config: "VllmConfig"):
        ...
        if self.spec_k_config.enabled and (
            ...
            raise ValueError(
                "Spec-K does not yet support EPLB or custom expert maps."
            )
...
class SpecKConfig:
    ...
    def __init__(self, config: dict[str, Any], vllm_config: "VllmConfig"):
        if not isinstance(config, dict):
            raise ValueError(
                "additional_config.spec_k_config must be a dict, "
                f"got {type(config).__name__}."
            )
        unknown_keys = set(config) - self._VALID_KEYS
        if unknown_keys:
            raise ValueError(
                "Unknown additional_config.spec_k_config keys: "
        ...
        if not isinstance(ppl_thresholds, (list, tuple)) or any(
            not isinstance(value, Real) or isinstance(value, bool)
            for value in ppl_thresholds
        ):
            raise ValueError(
                "spec_k_config.ppl_thresholds must be a list of numbers."
            )
        ...
        if not isinstance(layer_range, (list, tuple)) or len(layer_range) not in (
            ...
            raise ValueError(
                "spec_k_config.full_top_k_layer_range must contain 2 or 3 integers."
            )
        ...
        if any(
            ...
            raise ValueError(
                "spec_k_config.full_top_k_layer_range must contain only integers."
            )
        self.full_top_k_layer_range = tuple(int(value) for value in layer_range)
        if self.full_top_k_layer_range[2] == 0:
            raise ValueError(
                "spec_k_config.full_top_k_layer_range step must not be zero."
            )
        ...
        if not self.ppl_thresholds:
            raise ValueError(
                "spec_k_config.ppl_thresholds must not be empty when Spec-K is enabled."
            )
        if any(
            not math.isfinite(value) or value <= 0
            for value in self.ppl_thresholds
        ):
            raise ValueError(
                "spec_k_config.ppl_thresholds must contain finite, positive values."
            )
        if any(
            left < right
            for left, right in zip(
                self.ppl_thresholds, self.ppl_thresholds[1:]
            )
        ):
            raise ValueError(
                "spec_k_config.ppl_thresholds must be in non-increasing order."
            )
        ...
        if not self.supports_draft_logits(method):
            ...
            raise ValueError(
                "Spec-K requires a speculative method that provides draft logits; "
