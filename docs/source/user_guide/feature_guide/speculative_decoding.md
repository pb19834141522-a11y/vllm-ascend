...
Spec-K is an engine-global policy configured through `additional_config.spec_k_config`; it cannot be changed per request. It requires an unquantized or Ascend-quantized MoE target and a speculative method that returns draft logits. The draft proposer must run eagerly by setting `enforce_eager` to `true`; this does not prevent the target model from using ACL graph mode. The current implementation supports the V1 runner with pipeline parallel size 1, prefix caching and EPLB disabled, and no PCP or DCP. Asynchronous scheduling is supported only for fixed-length DSpark; dynamic speculative length must be disabled for that path. DP, TP, and expert parallelism use the AllGather MoE communication path; All2All, MC2, and FusedMC2 are not supported. 310P is not supported.
...
- Online inference
    ...
    vllm serve Qwen/Qwen3-30B-A3B \
      ...
      --speculative-config \
        '{"method": "draft_model", "model": "Qwen/Qwen3-0.6B", "num_speculative_tokens": 7, "enforce_eager": true}' \
      --additional-config \
        '{"spec_k_config": {"enabled": true, "ppl_thresholds": [16.226631, 16.226631, 11.861974, 11.861974, 7.394488]}}'
    ```
...
For threshold calibration, set `entropy_diagnostics_dir` inside `spec_k_config`. TP rank zero in every DP replica then writes one `spec_k_entropy_*.jsonl` file containing every proposed draft token's entropy, PPL, selected expert budget, and whether dynamic speculative decoding retained it for target verification. On DSpark, each record includes both the raw distribution and the distribution after Markov-head bias correction (`raw_entropy`, `corrected_entropy`, and `markov_entropy_delta`). The legacy `entropy` and `ppl` fields mean the corrected distribution consumed by Spec-K. This diagnostic mode performs an extra raw-logit softmax plus JSONL I/O and should not be used for final throughput measurements. Candidate thresholds and the Markov effect can be summarized without another inference run:
...
Use a new empty diagnostics directory for each measured dataset. The analyzer
reads every `spec_k_entropy_*.jsonl` file in the directory, including files
left by earlier service processes.
